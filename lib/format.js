...
/**
 * Normalize a payload's origin.
 *
 * Fails closed to 'system' so an unlabelled sender (an older hook, a runner that has not
 * been updated) is muted rather than allowed to notify. Muting a real session costs the
 * user a ping they can still find in the System tab; the reverse restores the noise
...
/**
 * Should this payload raise an OS notification (sound, alert, tray colour change)?
 *
 * Extracted as a pure predicate because it is the single decision this whole feature
 * turns on, and main.js cannot be unit-tested without mocking all of Electron.
 */
function shouldNotify(payload, systemMuted) {
  return !(normalizeOrigin(payload && payload.origin) === SYSTEM && systemMuted);
}
