...
const mockApp = {
  getPath() { return '/Applications/claude-tray-notifier.app/Contents/MacOS/claude-tray-notifier'; },
  exit() {}
};
...
describe('updater', () => {
  ...
  describe('setupAutoUpdater', () => {
    ...
    it('enables autoDownload and disables autoInstallOnAppQuit', () => {
      fs.writeFileSync(configPath, 'https://updates.example.com');
      timers = setupAutoUpdater();
  ...
  describe('checkForUpdatesManual', () => {
    it('shows warning dialog when no URL configured', () => {
  ...
  describe('event handlers', () => {
    it('update-available handler logs version', () => {
      fs.writeFileSync(configPath, 'https://updates.example.com');
      timers = setupAutoUpdater();
      // update-available just logs — should not throw
      assert.doesNotThrow(() => {
        mockAutoUpdater._handlers['update-available']({ version: '2.0.0' });
      });
    });

    it('update-downloaded shows restart dialog', async () => {
      fs.writeFileSync(configPath, 'https://updates.example.com');
      timers = setupAutoUpdater();
